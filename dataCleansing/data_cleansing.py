    ...
    for i, case in enumerate(cases):
        ...
        try:
            ...
            if pairs:
                ...
                log_message(
                    f' Found {len(pairs)} valid Q&A pairs in case {case_id}', 'GREEN'
                )
            else:
...
def main():
    ...
    if args.include_predefined:
        ...
        if predefined_pairs and save_predefined_csv(predefined_pairs):
            log_message(
                f' {len(predefined_pairs)} predefined Q&A pairs saved to predefined_csv.csv',
                'GREEN',
            )
    ...
    if text is not None:
        ...
        if num_cases > 0:
            ...
            log_message(
                f'Found {len(unique_qa_pairs)} unique Q&A pairs from cases', 'GREEN'
            )
        else:
