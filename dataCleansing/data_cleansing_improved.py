...
PATTERNS = {
    ...
    'question': re.compile(
        r'\[\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\]\s*\[\s*Ekstern\s*e-post\s*\]\s*(.*)'
    ),
    'answer': re.compile(
        r'\[\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}\]\s*(?!\[\s*Ekstern\s*e-post\s*\])(.*)'
    ),
}
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--debug', action='store_true', help='Skriv ut ekstra debug informasjon'
    )
...
def clean_text(text: str) -> str:
    ...
    text = re.sub(
        r'([.!?])\s+([A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+){1,3})\.?\s+(?:Rådgiver|Seniorrådgiver|Konsulent|Direktør|Leder|Sjef)\.?\s+[A-Za-zæøåÆØÅ]+\.?',
        r'\1',
        text,
    )
    ...
    text = re.sub(
        r'(?m)^([A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+){1,3})(?:\s*,\s*|\s+)((?:[A-ZÆØÅ][a-zæøå]+\s*)+)(?:\s*$|\s*\n)',
        '',
        text,
    )
    ...
    text = re.sub(
        r'([A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+){1,3})(?:Seniorrådgiver|Rådgiver|Konsulent|Direktør|Leder|Sjef)(?:,?\s*[A-Za-z]+)?',
        '',
        text,
    )
    ...
    text = re.sub(
        r'([A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+){1,2})(?:Seniorrådgiver|Rådgiver|Konsulent|Direktør|Leder|Sjef)',
        '',
        text,
    )
    ...
    text = re.sub(
        r'(?<=[a-zæøå\.\?\!])\s*[:;]?\s*\d{8}\b', '', text
    )  # Numbers after text
    text = re.sub(
        r'(?<=[a-zæøå\.\?\!])\s*[:;]?\s*\d{2}\s*\d{2}\s*\d{2}\s*\d{2}\b', '', text
    )  # Formatted numbers

    # Remove website URLs and phone numbers
    text = re.sub(r'(?i)(?:Telefon|Tlf|Mobil)[\s:]*[\d\s+()-]+', '', text)
    text = re.sub(
        r'(?i)(?:www\.)?[a-zæøå0-9][a-zæøå0-9-]{1,61}[a-zæøå0-9]\.[a-zæøå]{2,}(?:/[^\s]*)?',
        '',
        text,
    )
    ...
    text = re.sub(
        r'(?:\(?\+\d{2}\)?|\+\d{2}|00\d{2})[\s.-]*\d{1,2}[\s.-]*\d{1,2}[\s.-]*\d{1,2}[\s.-]*\d{1,2}[\s.-]*\d{0,2}',
        '',
        text,
    )
    ...
    original_message_pattern = (
        r'(?i)(?:[-]+\s*(?:Originalmelding|Original\s*melding)\s*[-]+)'
    )
    ...
    norway_phone_flexible = (
        r'\+47[\s.-]*\d{1,2}[\s.-]*\d{1,2}[\s.-]*\d{1,2}[\s.-]*\d{1,2}[\s.-]*\d{0,2}'
    )
    ...
    text = re.sub(
        r'(?i)(?:Kind\s+)?(?:Manager|Director|Assurance|Supply Chain|Product|Digital|Business|Consultant|Advisor|Rådgiver|Konsulent|Leder|Sjef)(?:[A-Za-z\s]+)?(?:\|\s*[a-z0-9.-]+\.(?:com|no|org|net)\s*\|\s*)?',
        '',
        text,
    )
    ...
    title_dept_pattern = (
        r'(?i)(?:Digital|Product|Supply Chain|Assurance|Platforms)[A-Za-z\s&]+'
    )
    text = re.sub(title_dept_pattern, '', text)

    # Enhanced contact information removal - improved patterns
    text = re.sub(
        r'(?i)(?:Kontaktinformasjon|Informasjon om innmelder)(?:[\s\S]*?)(?=\n\n|\Z)',
        '',
        text,
    )
    ...
    text = re.sub(
        r'(?i)(?:Fakturadresse|Fakturaadresse|Fakturareferanse|Fakturarefereanse)(?:[,:]|\s+)[^\n]+',
        '',
        text,
    )
    text = re.sub(
        r'(?i)(?:postboks|pb\.?|p\.?b\.?)(?:\s+\d+[A-Za-z0-9\s,./-]*)', '', text
    )
    text = re.sub(
        r'(?i)(?:Besøksadresse|Besøks-?adresse|Gateadresse|Forretningsadresse)(?:[,:]|\s+)[^\n]+',
        '',
        text,
    )
    ...
    text = re.sub(
        r'(?i)(?:[A-ZÆØÅ][a-zæøå]+(?:veien|gata|gaten|vegen|vei|gate|gt\.?|v\.?|plass|torg))\s+\d+\w?(?:[,\s]+\d{4}\s+[A-ZÆØÅ][a-zæøå]+)?',
        '',
        text,
    )
    ...
    text = re.sub(
        r'(?i)(?:Telefonnummer|Telefon|Tlf\.?|Mobil|Mobilnummer|Sentralbord|Tel\.?|Phone|Mobile)[\s:]*[\d\s+()-]{8,15}',
        '',
        text,
    )  # Standard format
    text = re.sub(
        r'(?<=[a-zæøå\.\?\!])\s*[:;]?\s*(?:\+\d{1,3}\s*)?(?:\d{2}\s*){4}\b', '', text
    )  # Formatted numbers after text
    text = re.sub(
        r'(?<=[a-zæøå\.\?\!])\s*[:;]?\s*(?:\+\d{1,3}\s*)?(?:\d{3}\s*){2,3}\b', '', text
    )  # 3-digit groupings
    ...
    text = re.sub(
        r'(?i)(?:linkedin|(?:www\.)?linkedin\.com|LinkedIn-profil).*?(?=\n|\Z)',
        '',
        text,
        flags=re.MULTILINE,
    )
    ...
    text = re.sub(
        r'(?i)(?:linkedin|(?:www\.)?linkedin\.com).*$', '', text, flags=re.MULTILINE
    )

    # Remove feedback/rating requests
    text = re.sub(
        r'(?i)Hvor\s+tilfreds\s+var\s+du\s+med\s+(?:responsen|svaret)[^\n]+', '', text
    )
    text = re.sub(r'(?i)gi\s+oss\s+tilbakemelding[^\n]*', '', text)
    text = re.sub(
        r'(?i)(?:takk\s+for\s+din\s+(?:henvendelse|forespørsel)|har\s+du\s+fått\s+svar\s+på\s+det\s+du\s+lurte\s+på)[^\n]*',
        '',
        text,
    )
    ...
    text = re.sub(
        r'(?m)^([A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+){1,2})[\.,!]?\s*$', '', text
    )
    ...
    text = re.sub(
        r'([A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+){1,2})\s*<[^>]+@[^>]+>', '', text
    )

    # Remove various greeting formats - use compiled patterns first (fastest)
    text = re.sub(
        r'(?i)(?:med\s+vennlig\s+hilsen|mvh|vennlig\s+hilsen|beste\s+hilsen|hilsen|regards)[\s,.]*(?:$|\n)',
        '',
        text,
    )
    text = re.sub(
        r'(?i)(?:\s*M\s*e\s*d\s+v\s*e\s*n\s*(?:n|l)\s*i\s*g\s*)(?:[\s,.]+h\s*i\s*l\s*s\s*e\s*n|[\s,.]+h(?:ä|a|e)l\s*s\s*(?:n|i\s*n\s*g)|[\s,.]+h\s*e\s*l\s*s\s*i\s*n\s*g)?[\s,.]*(?:$|\n)',
        '',
        text,
    )
    ...
    text = re.sub(
        r'(?i)(hilsen|regards|fra)[,:]?\s*[A-ZÆØÅa-zæøå]+\s+[A-ZÆØÅa-zæøå]+', '', text
    )
    ...
    jwt_extended_pattern = (
        r'eyJ[A-Za-z0-9_-]+\.(?:[A-Za-z0-9_-]+\.){1,10}[A-Za-z0-9_-]+'
    )
    ...
    social_media_pattern = (
        r'(?i)(?:facebook|twitter|instagram|linkedin|snapchat|tiktok)(?:\s+[\w\s]+)?'
    )
    text = re.sub(social_media_pattern, '', text)

    # Remove marketing material references
    marketing_pattern = (
        r'(?i)(?:brosjyre|flyer|poster|plakat|roll-up|banner|infographic|nyhetsbrev)'
    )
    ...
    text = re.sub(
        r'[A-Za-zæøåÆØÅ]+(?:direktoratet|departementet|etaten|instituttet)\.no',
        '',
        text,
    )
    ...
    date_pattern = (
        r'\w+\.\s+\d+\.\s+\w+\.\s+\d{4}\s+kl\.\s+\d{1,2}:\d{2}\s+skrev\s+[^:]+:'
    )
    ...
    text = re.sub(
        r'[A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+){0,3}\s*\|\s*\d{4}\s+[A-ZÆØÅ][a-zæøå]+\|\s*Direkte:\s*\d{3}\s*\d{2}\s*\d{3}',
        '',
        text,
    )
    ...
    text = re.sub(
        r'[A-ZÆØÅ][a-zæøå]+\s+[A-ZÆØÅ][a-zæøå]+\s+[A-ZÆØÅ][a-zæøå]+(?:kundeansvarlig|kundeservice|kontaktperson)(?:KFM|TEL|TLF)?:\s*\+\s*,\s*T:\s*\+\.?"',
        '',
        text,
    )
    ...
    text = re.sub(
        r'[A-ZÆØÅ][a-zæøå]+\s+[A-ZÆØÅ][a-zæøå]+\s+[A-ZÆØÅ][a-zæøå]+(?:Seksjons|Avdelings)?(?:\s*\|\s*\+\d{2}\s*\d{3}\s*\d{2}\s*\d{3})(?:[A-ZÆØÅ][A-Za-zæøåÆØÅ]+(?:Dokumentsenteret)?(?:\s*i\s*\|\s*\.)?)?',
        '',
        text,
    )
    ...
    text = re.sub(
        r'\s*•\s*(?:_+\s*)?(?:[A-ZÆØÅ][a-zæøå]+(?:(?:veien|gata|gaten|vegen|vei|gate|gt\.?|v\.?|plass|torg))\s+\d+\w?,?\s*\d{4}\s+[A-ZÆØÅa-zæøå]+)?',
        '',
        text,
    )
    ...
    text = re.sub(
        r'(?:[A-ZÆØÅ][a-zæøå]+),\s*(?:,\s*)?(?:[A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+)?)(?:,\s*\d+\.?)?',
        '',
        text,
    )
    ...
    text = re.sub(
        r'_{2,}\s*[A-ZÆØÅ][a-zæøå]+(?:\s+[A-ZÆØÅ][a-zæøå]+){1,5}\s*\d+', '', text
    )
    ...
    stray_json_prop = (
        r'(?:\"[a-zA-Z_]+\":\s*(?:\"[^\"]*\"|true|false|\d+|null)\s*,?\s*)'
    )
    ...
    empty_fields_pattern = (
        r'(?i)(?:Kontaktperson|Virksomheten|Organisasjonsnummer|Org\.nr)[:.]?\s*$'
    )
    ...
    for i, case in enumerate(cases):
        ...
        try:
            ...
            if pairs:
                ...
                log_message(
                    f'Found {len(pairs)} valid question-answer pairs in case {case_id}',
                    'GREEN',
                )
            else:
                log_message(
                    f'No valid question-answer pairs found in case {case_id}', 'YELLOW'
                )
            ...
            log_error(
                f'Error processing case {case_id}: {str(e)}', case[:100] if case else ''
            )
...
def save_predefined_csv(
    pairs: List[Tuple[str, str]], filename: str = 'predefined_csv.csv'
) -> bool:
    try:
        with open(filename, 'w', encoding='utf-8', newline='') as csvfile:
            csvfile.write('spm,svar\n')
...
def main() -> None:
    ...
    if args.include_predefined:
        ...
        log_message(
            f'Added {len(predefined_pairs)} predefined question-answer pairs', 'GREEN'
        )

        # Save predefined question-answer pairs to a separate CSV file
        if predefined_pairs and save_predefined_csv(predefined_pairs):
            log_message(
                f'{len(predefined_pairs)} predefined question-answer pairs saved to predefined_csv.csv',
                'GREEN',
            )
    ...
    if text is not None:
        ...
        if total_cases > 0:
            ...
            log_message(
                f'Found {len(unique_qa_pairs)} unique question-answer pairs from cases',
                'GREEN',
            )
        else:
